...
class RepositoryPort(Protocol):
    ...
    def delete_jobs(self, job_ids: list[str]) -> "Any": ...
    def delete_jobs_if_not_running(self, job_ids: list[str]) -> "Any": ...
    # delete_jobs_if_not_running 返回 DeleteJobsResult（domain.models）：
    # 任一 running 时整批不删除；不存在 id 幂等跳过。
    ...
    # 清理队列（加固计划 Task 11）：purge 事务内入队，sweeper 幂等消费。
