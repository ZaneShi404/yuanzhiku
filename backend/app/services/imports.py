"""Source and artifact ingestion orchestration."""

from __future__ import annotations

import io
import mimetypes
from pathlib import Path
from typing import BinaryIO

from app.adapters.sqlite import SqliteRepository
from app.adapters.storage import ArtifactStore
from app.domain.models import PasteImportRequest, SourceType


ALLOWED_SUFFIXES = {".pdf", ".docx", ".md", ".markdown", ".txt"}


class ImportService:
    def __init__(self, repository: SqliteRepository, artifacts: ArtifactStore) -> None:
        self.repository = repository
        self.artifacts = artifacts

    def paste(self, request: PasteImportRequest) -> dict:
        encoded = request.text.encode("utf-8")
        if len(encoded) > 10 * 1024 * 1024:
            raise ValueError("粘贴 UTF-8 文本不能超过 10MB")
        stored = self.artifacts.store_stream(io.BytesIO(encoded), len(encoded))
        source, version = self.repository.create_source_with_version(
            source_type=SourceType.PASTE.value,
            title=request.title,
            author=request.author,
            language=request.language,
            notes=request.notes,
            rights=request.rights.value,
            categories=request.categories,
            tags=request.tags,
            artifact_sha256=stored.sha256,
            original_name="pasted.md",
            media_type="text/markdown",
            byte_size=stored.byte_size,
        )
        job = self.repository.create_job("parse", source["id"], version["id"], stored.sha256, None, {"filename": "pasted.md", "media_type": "text/markdown"}, priority=100)
        self.repository.audit("paste_import", source["id"], "queued")
        return {"source": source, "content_version": version, "artifact": {"sha256": stored.sha256, "byte_size": stored.byte_size, "deduplicated": not stored.was_new}, "job": job}

    def file(
        self, stream: BinaryIO, filename: str, content_type: str | None, title: str, rights: str,
        author: str | None, language: str, notes: str | None, categories: list[str], tags: list[str], expected_bytes: int | None,
    ) -> dict:
        suffix = Path(filename).suffix.lower()
        if suffix not in ALLOWED_SUFFIXES:
            raise ValueError("仅支持 PDF、DOCX、Markdown 和 TXT")
        if not title.strip():
            title = Path(filename).stem or "未命名文档"
        stored = self.artifacts.store_stream(stream, expected_bytes)
        media_type = content_type or mimetypes.guess_type(filename)[0] or "application/octet-stream"
        source, version = self.repository.create_source_with_version(
            source_type=SourceType.FILE.value,
            title=title.strip(), author=author, language=language, notes=notes, rights=rights,
            categories=categories, tags=tags, artifact_sha256=stored.sha256, original_name=Path(filename).name,
            media_type=media_type, byte_size=stored.byte_size,
        )
        job = self.repository.create_job("parse", source["id"], version["id"], stored.sha256, None, {"filename": Path(filename).name, "media_type": media_type}, priority=100)
        self.repository.audit("file_import", source["id"], "queued")
        return {"source": source, "content_version": version, "artifact": {"sha256": stored.sha256, "byte_size": stored.byte_size, "deduplicated": not stored.was_new}, "job": job}
