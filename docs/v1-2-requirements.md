...
- **隧道段例外（决策 10，fake-IP 环境兼容）**：当且仅当目标主机名已通过注册域校验时，允许解析结果落在隧道段 `198.18.0.0/15` 或 `28.0.0.0/8`（代理工具 fake-IP/TUN 模式的常见隧道地址，公网不可路由）。安全论证：隧道段由本地 TUN 设备独占路由、映射到工具自身配置的真实目的地——攻击者无法用它把连接引向受害主机内部服务（经典 DNS 重绑定目标），主机名注册域白名单仍是第一道且唯一的域名控制；若环境无 fake-IP 工具，对隧道段的连接不可达并快速失败，无可用攻击面。其余保留段（100.64.0.0/10、169.254.0.0/16、文档段、多播段等）、回环与内网段**无条件拒绝**，不受此例外影响。属安全边界变更，须经独立审核门禁。
