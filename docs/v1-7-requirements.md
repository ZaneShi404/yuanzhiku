...
- 状态：**已审定（2026-09-04），实施计划已制定，实现未开始**。本文档为方案先行产物（用户 2026-09-04 拍板「方案文档先行」）；同日用户审定通过（§14：决策 27 作业形态与两个默认值确认），配套 `docs/v1-7-implementation-plan.md` 已制定；并入 `docs/requirements.md` 冻结基线随实现完成时执行，实现开工以实施计划确认为准。
