...
- **本地语音转写（默认路径，转写先行）**：视频导入即自动排队转写——FunASR/Paraformer 中文识别，模型显式下载、全程离线；远程转写端点作为可选降级路径，降级事实可审计
- **视频深度理解**：关键帧采样融合转写语义锚点（场景切换 + "语音说到什么就看那一刻" + 静音空档），转写完成后可一键「重新分析」获得引导帧（历史分析保留）；转写完整性判定为「可能缺失」时视频直送多模态模型（通义千问 / 小米 MiMo，支持分块直送与自备中转），直送不可行时自动按「关键帧联络表」兜底理解画面（可关闭），转写完整也可开启画面增强——画面理解条目逐条带时间定位、独立成表示进入证据链
...
- **AI 自动化（显式开启后）**：视频导入自动排队转写 → 分析 → 摘要（转写先行，转写不可用时分析退化为纯信号抽帧），文档/粘贴解析成功自动分类；AI 建议按「只填空缺」规则写入，用户已填字段绝不覆盖
